...
if($_SESSION['EDM_User'] == "meow"){
	...
	$sql = "SELECT ACMNo,SuccessMail,ErrorMail,WaitMail,Status,Create_date,Tital,OpenedMail FROM EDM.actionmail Order by Create_date";
}else{
	$sql_count = "SELECT count(*) FROM EDM.actionmail where OwnUser='".$_SESSION['EDM_User']."'";
	$sql = "SELECT ACMNo,SuccessMail,ErrorMail,WaitMail,Status,Create_date,Tital,OpenedMail FROM EDM.actionmail where OwnUser='".$_SESSION['EDM_User']."' Order by Create_date";
}
